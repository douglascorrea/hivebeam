defmodule Hivebeam.Claude do
  @moduledoc """
  Public API for distributed Claude ACP bridge calls.
  """
  ...
  def prompt(prompt, opts \\ []) when is_binary(prompt) do
    safe_local_call(fn -> ClaudeBridge.prompt(prompt, opts) end)
  end
  ...
  def prompt(target_node, prompt, opts) when is_atom(target_node) and is_binary(prompt) do
    if target_node == Node.self() do
      ...
      safe_remote_call(fn ->
        :erpc.call(target_node, ClaudeBridge, :prompt, [prompt, opts], timeout_ms)
      end)
    end
  end
  ...
  def status(nil, opts) do
    bridge_name = Keyword.get(opts, :bridge_name, CodexConfig.bridge_name("claude"))
    safe_local_call(fn -> ClaudeBridge.status(bridge_name) end)
  end
  ...
  def status(target_node, opts) when is_atom(target_node) do
    if target_node == Node.self() do
      ...
      safe_remote_call(fn ->
        :erpc.call(target_node, ClaudeBridge, :status, [bridge_name], timeout_ms)
      end)
    end
  end
  ...
  def cancel(nil, opts), do: safe_local_call(fn -> ClaudeBridge.cancel_prompt(opts) end)
  ...
  def cancel(target_node, opts) when is_atom(target_node) do
    if target_node == Node.self() do
      ...
      safe_remote_call(fn ->
        :erpc.call(target_node, ClaudeBridge, :cancel_prompt, [opts], timeout_ms)
      end)
    end
  end
