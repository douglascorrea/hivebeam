defmodule ACPex.Protocol.Session do
  ...
  def handle_info(message, state) do
    if function_exported?(state.handler_module, :handle_info, 2) do
      case apply(state.handler_module, :handle_info, [message, state.handler_state]) do
        {:noreply, new_handler_state} ->
          {:noreply, %{state | handler_state: new_handler_state}}
        ...
        other ->
          Logger.warning(
            "Session #{state.session_id}: Invalid #{inspect(state.handler_module)}.handle_info/2 return: #{inspect(other)}"
          )
      ...
      Logger.debug(
        "Session #{state.session_id}: Unhandled async message for #{inspect(state.handler_module)}: #{inspect(message)}"
      )
